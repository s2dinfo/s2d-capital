...
// MACRO (FRED)
async function fred(id:string) { if(!FR) return null; try { const r=await fetch(`https://api.stlouisfed.org/fred/series/observations?series_id=${id}&api_key=${FR}&file_type=json&sort_order=desc&limit=1`,{next:{revalidate:3600}}); if(!r.ok) return null; return (await r.json()).observations?.[0]; } catch{return null;} }
export async function fredChart(id:string,n=24) { if(!FR) return null; try { const r=await fetch(`https://api.stlouisfed.org/fred/series/observations?series_id=${id}&api_key=${FR}&file_type=json&sort_order=desc&limit=${n}`,{next:{revalidate:3600}}); if(!r.ok) return null; return (await r.json()).observations?.reverse().map((o:any)=>({date:new Date(o.date).toLocaleDateString('en-US',{month:'short',year:'2-digit'}),value:parseFloat(o.value)||0})); } catch{return null;} }
